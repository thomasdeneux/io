function filename = cfd_fn_getfile(varargin)
% function filename = cfd_fn_getfile(varargin)
% returns file name with full path
% currently uses spm_select instead of UIGETFILE

swd=pwd;
rep = fn_getfile('REP'); % use the persistent directory name stored in fn_getfile
if ~isempty(rep), try cd(rep), catch end, end

arg = varargin;
saveflag=0;
if (length(arg)>=1 && ischar(arg{1}) && strcmp(arg{1},'save')), saveflag=1; arg={arg{2:end}}; end
...
if saveflag, 
    ...
    if 1 % strcmp(getenv('HOSTNAME'),'brainstorm') 
        % uigetfile not good in Amiram's lab - use spm_select instead
        [filt prompt] = deal(arg{1:2});
        if ~ischar(filt), filt='*.*'; end
        filt = ['^' filt '$'];
        filt = strrep(filt,'^*','');
        filt = strrep(filt,'*$','');
        filename = cfd_spm_select2(Inf,filt,prompt,'',pwd,filt);
        if isempty(filename) filename=''; return; end
        pathname = fileparts(filename(1,:));
    else
        [filename pathname] = uigetfile(arg{:});
        filename = fullfile(pathname,filename);
    end
end
...
if filename
    rep = pathname;
    fn_getfile('REP',rep) % set the persistent directory name in fn_getfile
end
...
%=======================================================================
function [t,sts] = cfd_spm_select2(varargin)
% File selector
...
if nargin > 0 && ischar(varargin{1})
    ...
    for i=1:size(t,1)
        fname = t(i,:);
        f = findstr(fname,' [');
        if ~isempty(f), t(i,:) = [fname(1:f-1) repmat(' ',1,size(t,2)-f+1)]; end
    end
end
%=======================================================================
