function cfd_fn_getfile_keep(filt,m,fhandle)
% function cfd_fn_getfile_keep(filt,m,fhandle)
%---
% the function 'cfd_fn_getfile_keep' is special: 
% whenever a file will be selected, it will call 'fhandle' on it, where
% fhandle is normally the handle of function cfdm_open(dum1,dum2,m,fname)

% persistent rep
% 
% swd=pwd;
% if ~isempty(rep), try cd(rep), catch end, end

prompt='Select file';

if ~ischar(filt), filt='*.*'; end
filt = ['^' filt '$'];
filt = strrep(filt,'^*','');
filt = strrep(filt,'*$','');
% let's start in fn_getfile persistent working directory
rep = fn_getfile('REP');
if isempty(rep), rep = pwd; end
try swd = pwd; cd(rep), cd(swd), catch rep = pwd; end
cfd_spm_select3(m,fhandle,Inf,filt,prompt,'',rep,filt);

%=======================================================================

%=======================================================================
function rep = cfd_spm_select3(m,fhandle,varargin)
% File selector
...
if nargin > 2 && ischar(varargin{1})
    switch lower(varargin{1})
        case 'addvfiles'
            error(nargchk(2,Inf,nargin-2));
            vfiles('add',varargin{2:end});
        case 'clearvfiles'
            error(nargchk(1,1,nargin-2));
            vfiles('clear');
        case 'vfiles'
            error(nargchk(1,1,nargin-2));
            t = vfiles('all');
        ...
        case 'list'
            ...
            if fn_debug
                disp('check what i''m doing here please')
                keyboard
            end
        otherwise 
    ...
    selector(m,fhandle,varargin{:});
end
%=======================================================================

%=======================================================================
function selector(m,fhandle,n,typ,mesg,already,wd,filt,frames)
if nargin<9, frames  = '1';     end;
if nargin<8, filt    = '.*';    end;
if nargin<7, wd      = pwd;     end;
if nargin<6, already = {''};    end;
if nargin<5, mesg    = 'Select files...'; end;
if nargin<4, typ     = 'any';   end;
if nargin<3, n       = [0 Inf]; end;
ok  = 0;
...
% make a new figure only if there is no selection figure opened yet
fg = findobj('type','figure','tag','Select');
while ~isempty(fg)
    if isappdata(fg(1),'cfd_menu')
        figure(fg(1))
        return
    else
        fg(1) = [];
...
tmp = uicontrol(fg,...
    'style','listbox',...
    'units','normalized',...
    'Position',[0.36 hp 0.62 h2],...
    'FontSize',fs,...
    'Callback',{@click_file_box,m,fhandle},...
    'tag','files',...
    'BackgroundColor',col1,...
    'ForegroundColor',col3,...
    'UserData',n,...
...
obregexp = sib(ob,'regexp');
set(obregexp,'String','.*');
ud = get(obregexp,'UserData');
ud.ext = {get(obregexp,'String')};
set(obregexp,'UserData',ud)
update(ob);
return;
%=======================================================================
...
rep = current_dir(lb);
% set the persistent directory in fn_getfile to this new value
fn_getfile('REP',rep)
update(lb,rep);
return;
%=======================================================================
...
set(sib(lb,'files'),'Value',1,'String',getattributes(dr,f));
[ls,mch] = prevdirs(dr);
...
%=======================================================================
function click_file_box(lb,dum2,m,fhandle)
lim  = get(lb,'UserData');
ob   = sib(lb,'selected');
str3 = get(ob,'String');
...
% FINAL ACTION: call fhandle
feval(fhandle,[],[],m,fname)
...
%=======================================================================
%             FUNCTIONS ADDED BY THOMAS DENEUX                         %
%=======================================================================

function str = getattributes(dr,f)

str = [];
for i=1:size(f,1)
    fname = f(i,:);
    header = cfd_headers(deblank(fullfile(dr,fname)));
    if header.ACV.ZStepSize, type='ZStck'; else type='Time'; end
    attributes = sprintf(' [%s %ix%ix%i X%.2f]', ...
        type,header.ACV.pixels.x,header.ACV.pixels.y,header.ACV.num.images,header.ASV.zoom_fac);
    str = strvcat(str,[fname attributes]);
end

%---
function closefig(dum1,dum2,fg)
